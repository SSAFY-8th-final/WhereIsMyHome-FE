...
export default {
   ...
   computed: {
      storeTitle: {
         get() {
            return this.$store.state.notice.title;
         },
         set(title) {
            this.$store.commit("SET_NOTICE_TITLE", title);
         },
      },
   },
   methods: {
      ...
      async noticeUpdate() {
         ...
         try {
            let { data } = await http.put("/admins/notices/" + this.$store.state.notice.noticeId, noticeObj);
